...
<c:import url="../layout/app.jsp">
    ...
    <h2>フォロワーの従業員　一覧</h2>
        <table id="employee_list">
            <tbody>
                ...
                <c:forEach var="employee" items="${follower}" varStatus="status">
                    <tr class="row${status.count % 2}">
                        <td><c:out value="${employee.employee_id.code}" /></td>
                        <td><c:out value="${employee.employee_id.name}" /></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
        ...
        <div id="pagination">
            （全 ${sessionScope.follower_count} 件）<br />
            <c:forEach var="i" begin="1" end="${((sessionScope.follower_count - 1) / maxRow) + 1}" step="1">
                <c:choose>
                    <c:when test="${i == page}">
                        <c:out value="${i}" />&nbsp;
                    </c:when>
    ...
    <table id="report_list">
            <tbody>
                ...
                <c:forEach var="report" items="${follower_reports}" varStatus="status">
        ...
        <div id="pagination">
            ...
            <c:forEach var="i" begin="1" end="${((follower_reports_count - 1) / maxRow) + 1}" step="1">
                <c:choose>
                    <c:when test="${i == page}">
                        <c:out value="${i}" />&nbsp;
                    </c:when>
